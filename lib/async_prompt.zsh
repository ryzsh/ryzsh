...
# with _ryz_register_handler:
#
#  function _git_prompt_status_async {
#    # Do some expensive operation that outputs to stdout
#  }
#  _ryz_register_handler _git_prompt_status_async
#
# Then add a stub prompt function in `$PROMPT` or similar prompt variables,
# which will show the output of "$_RYZ_ASYNC_OUTPUT[handler_name]":
#
#  function git_prompt_status {
#    echo -n $_RYZ_ASYNC_OUTPUT[_git_prompt_status_async]
#  }
#
...
function _ryz_register_handler {
  setopt localoptions noksharrays
  typeset -ga _ryz_async_functions
  # we want to do nothing if there's no $1 function or we already set it up
  if [[ -z "$1" ]] || (( ! ${+functions[$1]} )) \
    || (( ${_ryz_async_functions[(Ie)$1]} )); then
    return
  fi
  _ryz_async_functions+=("$1")
  # let's add the hook to async_request if it's not there yet
  if (( ! ${precmd_functions[(Ie)_ryz_async_request]} )) \
    && (( ${+functions[_ryz_async_request]})); then
    autoload -Uz add-zsh-hook
    add-zsh-hook precmd _ryz_async_request
  fi
}

# Set up async handlers and callbacks
function _ryz_async_request {
  local -i ret=$?
  typeset -gA _RYZ_ASYNC_FDS _RYZ_ASYNC_PIDS _RYZ_ASYNC_OUTPUT

  # executor runs a subshell for all async requests based on key
  local handler
  for handler in ${_ryz_async_functions}; do
    (( ${+functions[$handler]} )) || continue

    local fd=${_RYZ_ASYNC_FDS[$handler]:--1}
    local pid=${_RYZ_ASYNC_PIDS[$handler]:--1}
    ...
    _RYZ_ASYNC_FDS[$handler]=-1
    _RYZ_ASYNC_PIDS[$handler]=-1
    ...
    read -u $fd "_RYZ_ASYNC_PIDS[$handler]"

    # When the fd is readable, call the response handler
    zle -F "$fd" _ryz_async_callback
  done
}

# Called when new data is ready to be read from the pipe
function _ryz_async_callback() {
  emulate -L zsh
  ...
  if [[ -z "$err" || "$err" == "hup" ]]; then
    ...
    local handler="${(k)_RYZ_ASYNC_FDS[(r)$fd]}"

    # Store old output which is supposed to be already printed
    local old_output="${_RYZ_ASYNC_OUTPUT[$handler]}"

    # Read output from fd
    IFS= read -r -u $fd -d '' "_RYZ_ASYNC_OUTPUT[$handler]"

    # Repaint prompt if output has changed
    if [[ "$old_output" != "${_RYZ_ASYNC_OUTPUT[$handler]}" ]]; then
      zle .reset-prompt
      zle -R
    fi
