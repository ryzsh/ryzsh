...
function ryz {
  ...
  # appear as completion entries when looking for `ryz`
  (( ${+functions[_ryz::$command]} )) || {
    _ryz::help
    return 1
  }
...
function _ryz::log {
  ...
  if [[ $logtype = debug && -z $_RYZ_DEBUG ]]; then
    return
  fi
...
Available commands:
  ...
  pr     <command>    Manage RyZsh Pull Requests
  reload              Reload the current zsh session
...
Available commands:
  ...
  list             List all available RyZsh plugins
...
function _ryz::plugin::disable {
  ...
  for plugin in "$@"; do
    if [[ ${plugins[(Ie)$plugin]} -eq 0 ]]; then
      _ryz::log warn "plugin '$plugin' is not enabled."
      continue
    fi
    dis_plugins+=("$plugin")
  done
...
"
  ...
  [[ $? -eq 0 ]] || {
    local ret=$?
    _ryz::log error "error disabling plugins."
    return $ret
  }

  # Exit if the new .zshrc file has syntax errors
  if ! command zsh -n "$zdot/.zshrc"; then
    _ryz::log error "broken syntax in '"${zdot/#$HOME/\~}/.zshrc"'. Rolling back changes..."
    command mv -f "$zdot/.zshrc.bck" "$zshrc"
    return 1
  fi

  # Restart the zsh session if there were no errors
  _ryz::log info "plugins disabled: ${(j:, :)dis_plugins}."

  # Only reload zsh if run in an interactive session
  [[ ! -o interactive ]] || _ryz::reload
}

function _ryz::plugin::enable {
  if [[ -z "$1" ]]; then
    echo >&2 "Usage: ${(j: :)${(s.::.)0#_}} <plugin> [...]"
    return 1
  fi
  ...
  for plugin in "$@"; do
    if [[ ${plugins[(Ie)$plugin]} -ne 0 ]]; then
      _ryz::log warn "plugin '$plugin' is already enabled."
      continue
    fi
    add_plugins+=("$plugin")
  done
...
"
  ...
  [[ $? -eq 0 ]] || {
    local ret=$?
    _ryz::log error "error enabling plugins."
    return $ret
  }
  ...
  # Restart the zsh session if there were no errors
  _ryz::log info "plugins enabled: ${(j:, :)add_plugins}."

  # Only reload zsh if run in an interactive session
  [[ ! -o interactive ]] || _ryz::reload
}

function _ryz::plugin::info {
  if [[ -z "$1" ]]; then
    echo >&2 "Usage: ${(j: :)${(s.::.)0#_}} <plugin>"
    return 1
  fi
  ...
  if [[ -d "$ZSH_CUSTOM/plugins/$1" || -d "$ZSH/plugins/$1" ]]; then
    _ryz::log error "the '$1' plugin doesn't have a README file"
  else
    _ryz::log error "'$1' plugin not found"
  fi

  return 1
}

function _ryz::plugin::list {
...
function _ryz::plugin::load {
  ...
  for plugin in "$@"; do
    if [[ -d "$ZSH_CUSTOM/plugins/$plugin" ]]; then
      ...
      _ryz::log warn "plugin '$plugin' not found"
      continue
    fi

    # Check if its a valid plugin
    if [[ ! -f "$base/_$plugin" && ! -f "$base/$plugin.plugin.zsh" ]]; then
      _ryz::log warn "'$plugin' is not a valid plugin"
      continue
...
Available commands:
  ...
  clean                       Delete all PR branches (ryzsh/pull-*)
...
function _ryz::pr::clean {
  (
    ...
    branches="$(command git for-each-ref --sort=-committerdate --color --format="$fmt" "refs/heads/ryzsh/pull-*")"

    # Exit if there are no PR branches
    if [[ -z "$branches" ]]; then
      _ryz::log info "there are no Pull Request branches to remove."
      return
    fi
    ...
    _ryz::confirm "do you want remove these Pull Request branches? [Y/n] "
    # Only proceed if the answer is a valid yes option
    [[ "$REPLY" != [yY$'\n'] ]] && return

    _ryz::log info "removing all RyZsh Pull Request branches..."
    command git branch --list 'ryzsh/pull-*' | while read branch; do
      command git branch -D "$branch"
    done
  )
}

function _ryz::pr::test {
  ...
  branch=$(builtin cd -q "$ZSH"; git symbolic-ref --short HEAD) || {
    _ryz::log error "error when getting the current git branch. Aborting..."
    return 1
  }


  # Fetch PR onto ryzsh/pull-<PR_NUMBER> branch and rebase against master
  # If any of these operations fail, undo the changes made
  (
    set -e
    builtin cd -q "$ZSH"

    # Get the ryzsh git remote
    command git remote -v | while read remote url _; do
      case "$url" in
      https://github.com/ryzsh/ryzsh(|.git)) found=1; break ;;
      git@github.com:ryzsh/ryzsh(|.git)) found=1; break ;;
      esac
    done

    (( $found )) || {
      _ryz::log error "could not found the ryzsh git remote. Aborting..."
      return 1
    }

    # Fetch pull request head
    _ryz::log info "fetching PR #$1 to ryzsh/pull-$1..."
    command git fetch -f "$remote" refs/pull/$1/head:ryzsh/pull-$1 || {
      _ryz::log error "error when trying to fetch PR #$1."
      return 1
    }
    ...
    local ret gpgsign
    {
      ...
      command git rebase master ryzsh/pull-$1 || {
        command git rebase --abort &>/dev/null
        _ryz::log warn "could not rebase PR #$1 on top of master."
    ...
    _ryz::log info "fetch of PR #${1} successful."
  )
  ...
  _ryz::log info "running \`zsh\` to test the changes. Run \`exit\` to go back."
  command zsh -l

  # After testing, go back to the previous HEAD if the user wants
  _ryz::confirm "do you want to go back to the previous branch? [Y/n] "
  # Only proceed if the answer is a valid yes option
  [[ "$REPLY" != [yY$'\n'] ]] && return
  ...
  (
    ...
    command git checkout "$branch" -- || {
      _ryz::log error "could not go back to the previous branch ('$branch')."
      return 1
    }
  )
}

function _ryz::reload {
...
Available commands:
  ...
  list            List all available RyZsh themes
...
function _ryz::theme::set {
  ...
  if [[ ! -f "$ZSH_CUSTOM/$1.zsh-theme" ]] \
    ...
    _ryz::log error "%B$1%b theme not found"
    return 1
  fi
...
!set && /^[ \t]*ZSH_THEME=[^#]+.*$/ {
  set=1
  sub(/^[ \t]*ZSH_THEME=[^#]+.*$/, "ZSH_THEME=\"'$1'\" # set by `ryz`")
  print $0
  next
}
...
EOF
  ...
  [[ $? -eq 0 ]] || {
    local ret=$?
    _ryz::log error "error setting theme."
    return $ret
  }
  ...
  # Restart the zsh session if there were no errors
  _ryz::log info "'$1' theme set correctly."

  # Only reload zsh if run in an interactive session
  [[ ! -o interactive ]] || _ryz::reload
}

function _ryz::theme::use {
  if [[ -z "$1" ]]; then
    echo >&2 "Usage: ${(j: :)${(s.::.)0#_}} <theme>"
    return 1
  fi
...
function _ryz::update {
  # Check if git command is available
  (( $+commands[git] )) || {
    _ryz::log error "git is not installed. Aborting..."
    return 1
  }

  local last_commit=$(builtin cd -q "$ZSH"; git rev-parse HEAD 2>/dev/null)
  [[ $? -eq 0 ]] || {
    _ryz::log error "\`$ZSH\` is not a git directory. Aborting..."
    return 1
  }

  # Run update script
  zstyle -s ':ryz:update' verbose verbose_mode || verbose_mode=default
