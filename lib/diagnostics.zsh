# diagnostics.zsh
#
# Diagnostic and debugging support for ry-zsh
...
# ry-zsh, zsh, and the user's system. This is intended to provide a
...
#
# This is intended to be widely portable, and run anywhere that ry-zsh does.
...
#       Verbosity levels:
#        0 - Basic info, shell state, ryz configuration, git state
...
function ryz_diagnostic_dump() {
  emulate -L zsh

  builtin echo "Generating diagnostic dump; please be patient..."

  local thisfcn=ryz_diagnostic_dump
  local -A opts
  local opt_verbose opt_noverbose opt_outfile
  local timestamp=$(date +%Y%m%d-%H%M%S)
  local outfile=ryz_diagdump_$timestamp.txt
  ...
  # captured cleanly
  _ryz_diag_dump_one_big_text &> "$outfile"
  if [[ $? != 0 ]]; then
  ...
  builtin echo
  builtin echo To share this with RYZ developers, post it as a gist on GitHub
  builtin echo at "https://gist.github.com" and share the link to the gist.
  builtin echo
  builtin echo "WARNING: This dump file contains all your zsh and ryz configuration files,"
  builtin echo "so don't share it publicly if there's sensitive information in them."
  builtin echo

}

function _ryz_diag_dump_one_big_text() {
  local program programs progfile md5

  builtin echo ry-zsh diagnostic dump
  builtin echo
  ...
  builtin echo
  _ryz_diag_dump_os_specific_version
  builtin echo
  ...
  _ryz_diag_dump_check_core_commands || return 1
  builtin echo
  ...
  builtin set | command grep -a '^\(ZSH\|plugins\|TERM\|LC_\|LANG\|precmd\|chpwd\|preexec\|FPATH\|TTY\|DISPLAY\|PATH\)\|RYZ'
  builtin echo
  ...
  builtin echo ry-zsh installation:
  command ls -ld ~/.z*
  command ls -ld ~/.oh*
  builtin echo
  builtin echo ry-zsh git state:
  ...
  if [[ -e $ZSH_CUSTOM ]]; then
    ...
    builtin echo "ry-zsh custom dir:"
  ...
  if [[ $verbose -ge 1 ]]; then
    for cfgfile in $cfgfiles; do
      _ryz_diag_dump_echo_file_w_header $cfgfile
    done
  fi
  builtin echo
  ...
  if [[ $verbose -ge 2 ]]; then
    for dumpfile in $dumpfiles; do
      _ryz_diag_dump_echo_file_w_header $dumpfile
    done
  fi

}

function _ryz_diag_dump_check_core_commands() {
  ...
  # "history" is also excluded because RYZ is known to redefine that
...
function _ryz_diag_dump_echo_file_w_header() {
  local file=$1
...
function _ryz_diag_dump_os_specific_version() {
