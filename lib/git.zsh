...
function _ryz_git_prompt_info() {
  # If we are on a folder not tracked by git, get out.
  # Otherwise, check for hide-info at global and local repository level
  if ! __git_prompt_git rev-parse --git-dir &> /dev/null \
    || [[ "$(__git_prompt_git config --get ry-zsh.hide-info 2>/dev/null)" == 1 ]]; then
    return 0
  fi
...
# - https://github.com/ryzsh/ryzsh/issues/12331
# - https://github.com/ryzsh/ryzsh/issues/12360
# TODO(2024-06-12): @mcornella remove workaround when CentOS 7 reaches EOL
local _style
if zstyle -t ':ryz:alpha:lib:git' async-prompt \
  || { is-at-least 5.0.6 && zstyle -T ':ryz:alpha:lib:git' async-prompt }; then
  function git_prompt_info() {
    if [[ -n "${_RYZ_ASYNC_OUTPUT[_ryz_git_prompt_info]}" ]]; then
      echo -n "${_RYZ_ASYNC_OUTPUT[_ryz_git_prompt_info]}"
    fi
  }

  function git_prompt_status() {
    if [[ -n "${_RYZ_ASYNC_OUTPUT[_ryz_git_prompt_status]}" ]]; then
      echo -n "${_RYZ_ASYNC_OUTPUT[_ryz_git_prompt_status]}"
    fi
  }
  ...
  function _defer_async_git_register() {
    ...
    *(\$\(git_prompt_info\)|\`git_prompt_info\`)*)
      _ryz_register_handler _ryz_git_prompt_info
      ;;
    esac

    case "${PS1}:${PS2}:${PS3}:${PS4}:${RPROMPT}:${RPS1}:${RPS2}:${RPS3}:${RPS4}" in
    *(\$\(git_prompt_status\)|\`git_prompt_status\`)*)
      _ryz_register_handler _ryz_git_prompt_status
      ;;
    esac
...
elif zstyle -s ':ryz:alpha:lib:git' async-prompt _style && [[ $_style == "force" ]]; then
  function git_prompt_info() {
    if [[ -n "${_RYZ_ASYNC_OUTPUT[_ryz_git_prompt_info]}" ]]; then
      echo -n "${_RYZ_ASYNC_OUTPUT[_ryz_git_prompt_info]}"
    fi
  ...
  function git_prompt_info() {
    _ryz_git_prompt_info
  }
  function git_prompt_status() {
    _ryz_git_prompt_status
  }
fi
...
function parse_git_dirty() {
  ...
  local -a FLAGS
  FLAGS=('--porcelain')
  if [[ "$(__git_prompt_git config --get ry-zsh.hide-dirty)" != "1" ]]; then
...
function _ryz_git_prompt_status() {
  [[ "$(__git_prompt_git config --get ry-zsh.hide-status 2>/dev/null)" = 1 ]] && return
