"""
Update Emoji.py
Refreshes RYZ emoji database based on the latest Unicode spec
"""
import re
import json
...
headers = """
# emoji-char-definitions.zsh - Emoji definitions for ry-zsh emoji plugin
#
...
# def country_iso(_all_names, _ryz_name):
...
#     ryz_no_underscore = re.sub(r'_', r' ', _ryz_name)
#     iso2 = cc.convert(names=[ryz_no_underscore], to='ISO2')
#     if iso2 != 'not found':
#         _all_names.append(iso2)
#         iso3 = cc.convert(names=[ryz_no_underscore], to='ISO3')
...
for line in spec:
    ...
    if emoji_match != []:
        ...
        ryz_codes = code_to_ryz(code_points)
        ryz_name = name_to_ryz(name, group, subgroup, status)
        # If this emoji has the same shortname as the preceding one
        if ryz_name in short_name_buffer:
            ryz_name = increment_name(short_name_buffer)
        short_name_buffer = ryz_name
        emoji_database.append(
            [ryz_codes, status, emoji, ryz_name, group, subgroup])
spec.close()
...
all_ryz_names = [emoji_data[3] for emoji_data in emoji_database]
...
for _ryz_codes, _status, _emoji, _ryz_name, _group, _subgroup in emoji_database:

    # One emoji can be mapped to multiple names (aliases or country codes)
    names_for_this_emoji = [_ryz_name]
