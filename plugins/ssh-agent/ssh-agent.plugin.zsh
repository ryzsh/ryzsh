...
function _start_agent() {
  ...
  if [[ ! -d "$HOME/.ssh" ]]; then
    echo "[ry-zsh] ssh-agent plugin requires ~/.ssh directory"
    return 1
  fi

  # Set a maximum lifetime for identities added to ssh-agent
  local lifetime
  zstyle -s :ryz:plugins:ssh-agent lifetime lifetime

  # start ssh-agent and setup environment
  zstyle -t :ryz:plugins:ssh-agent quiet || echo >&2 "Starting ssh-agent ..."
...
function _add_identities() {
  ...
  zstyle -a :ryz:plugins:ssh-agent identities identities
  ...
  local args
  zstyle -a :ryz:plugins:ssh-agent ssh-add-args args

  # if ssh-agent quiet mode, pass -q to ssh-add
  zstyle -t :ryz:plugins:ssh-agent quiet && args=(-q $args)

  # use user specified helper to ask for password (ksshaskpass, etc)
  local helper
  zstyle -s :ryz:plugins:ssh-agent helper helper
...
if zstyle -t :ryz:plugins:ssh-agent agent-forwarding \
...
if ! zstyle -t :ryz:plugins:ssh-agent lazy; then
  _add_identities
fi
