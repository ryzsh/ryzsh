...
function xc {
  ...
  # See https://github.com/ryzsh/ryzsh/issues/10384
...
function xcselv {
  ...
  if [[ $# == 0 ]]; then
    ...
    _ryz_xcode_print_active_version
    return
  elif [[ $1 == "-l" ]]; then
    _ryz_xcode_list_versions
    return
  elif [[ $1 == "-L" ]]; then
    _ryz_xcode_list_versions short
    return
  elif [[ $1 == "-h" ]]; then
    _ryz_xcode_print_xcselv_usage
    return 0
  elif [[ $1 == -* && $1 != "-" ]]; then
...
function _ryz_xcode_print_xcselv_usage {
  cat << EOF >&2
Usage:
  xcselv <version>
  xcselv [options]
...
# Only meaningful when called from other _ryz_xcode functions
function _ryz_xcode_parse_versioned_file {
  local file=$1
...
function _ryz_xcode_print_active_version {
  emulate -L zsh
  local -A xcode_versions
...
function _ryz_xcode_locate_versions {
  emulate -L zsh
  local -a app_dirs
  ...
  for app_dir ($app_dirs); do
    ...
    for app ($apps); do
      xcode_ver=$(_ryz_xcode_parse_versioned_file $app)
      if [[ $? != 0 ]]; then
        continue
      fi
...
function _ryz_xcode_list_versions {
  emulate -L zsh
  local -A xcode_versions
  _ryz_xcode_locate_versions
