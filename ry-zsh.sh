...
# Protect against non-zsh execution of RyZsh (use POSIX syntax here)
[ -n "$ZSH_VERSION" ] || {
  ryz_ptree() {
...

  {
    ...
    printf "$(ryz_f 1 31)Error:$(ryz_f 22) RyZsh can't be loaded from: $(ryz_f 1)${shell}$(ryz_f 22). "
    printf "You need to run $(ryz_f 1)zsh$(ryz_f 22) instead.$(ryz_f 0)\n"
    printf "$(ryz_f 33)Here's the process tree:$(ryz_f 22)\n\n"
    ryz_ptree
    printf "$(ryz_f 0)\n"
  } >&2
...
# https://github.com/ryzsh/ryzsh/issues/11686
[[ "$(emulate)" = zsh ]] || {
  printf "$(ryz_f 1 31)Error:$(ryz_f 22) RyZsh can't be loaded in \`$(emulate)\` emulation mode.$(ryz_f 0)\n" >&2
  return 1
}
...
if [[ ! -w "$ZSH_CACHE_DIR" ]]; then
  ZSH_CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/ry-zsh"
fi
...
for plugin ($plugins); do
  if is_plugin "$ZSH_CUSTOM" "$plugin"; then
    ...
    echo "[ry-zsh] plugin '$plugin' not found"
  fi
done
...
zcompdump_revision="#ryz revision: $(builtin cd -q "$ZSH"; git rev-parse HEAD 2>/dev/null)"
zcompdump_fpath="#ryz fpath: $fpath"

# Delete the zcompdump file if RYZ zcompdump metadata changed
...
if (( $zcompdump_refresh )) \
  ...
  # See https://github.com/ryzsh/ryzsh/commit/dd1a7269#commitcomment-39003489
...
_ryz_source() {
  ...
  zstyle -T ":ryz:${context}" aliases || disable_aliases=1
...
# Load all of the lib files in ~/ry-zsh/lib that end in .zsh
# TIP: Add files you don't want in git to .gitignore
for lib_file ("$ZSH"/lib/*.zsh); do
  _ryz_source "lib/${lib_file:t}"
done
unset lib_file

# Load all of the plugins that were defined in ~/.zshrc
for plugin ($plugins); do
  _ryz_source "plugins/$plugin/$plugin.plugin.zsh"
done
unset plugin
