...
zstyle -s ':ryz:update' mode update_mode || {
  update_mode=prompt
...
function is_update_available() {
  local branch
  branch=${"$(builtin cd -q "$ZSH"; git config --local ry-zsh.branch)":-master}

  local remote remote_url remote_repo
  remote=${"$(builtin cd -q "$ZSH"; git config --local ry-zsh.remote)":-origin}
...
function handle_update() {
  () {
    ...
    trap "
      ret=\$?
      unset update_mode
      unset -f current_epoch is_update_available update_last_updated_file update_ryzsh handle_update 2>/dev/null
      command rm -rf '$ZSH/log/update.lock'
      return \$ret
    " EXIT INT QUIT
    ...
    zstyle -s ':ryz:update' frequency epoch_target || epoch_target=${UPDATE_ZSH_DAYS:-13}
    # Test if enough time has passed until the next update
    if (( ( $(current_epoch) - $LAST_EPOCH ) < $epoch_target )); then
      return
    fi

    # Test if RyZsh directory is a git repository
    if ! (builtin cd -q "$ZSH" && LANG= git rev-parse &>/dev/null); then
      echo >&2 "[ry-zsh] Can't update: not a git repository."
      return
    fi
    ...
    if [[ "$update_mode" = reminder ]] || { [[ "$update_mode" != background-alpha ]] && has_typed_input }; then
      ...
      echo "[ry-zsh] It's time to update! You can do that by running \`ryz update\`"
      return 0
    fi
    ...
    echo -n "[ry-zsh] Would you like to update? [Y/n] "
    read -r -k 1 option
    [[ "$option" = $'\n' ]] || echo
    case "$option" in
      [yY$'\n']) update_ryzsh ;;
      [nN]) update_last_updated_file ;&
      *) echo "[ry-zsh] You can update manually by running \`ryz update\`" ;;
    esac
  }

  unset update_mode
  unset -f current_epoch is_update_available update_last_updated_file update_ryzsh handle_update
}
...
case "$update_mode" in
  background-alpha)
    ...
    _ryz_bg_update() {
      ...
      add-zsh-hook precmd _ryz_bg_update_status

      # deregister background function
      add-zsh-hook -d precmd _ryz_bg_update
      unset -f _ryz_bg_update
    }

    _ryz_bg_update_status() {
      {
        ...
        if [[ "$EXIT_STATUS" -eq 0 ]]; then
          print -P "\n%F{green}[ry-zsh] Update successful.%f"
          return 0
        elif [[ "$EXIT_STATUS" -ne 0 ]]; then
          print -P "\n%F{red}[ry-zsh] There was an error updating:%f"
        ...
        if (( TRY_BLOCK_ERROR == 0 )); then
          ...
          # deregister background function
          add-zsh-hook -d precmd _ryz_bg_update_status
          unset -f _ryz_bg_update_status
        fi
      }
    }

    add-zsh-hook precmd _ryz_bg_update
    ;;
  *)
    handle_update ;;
esac
